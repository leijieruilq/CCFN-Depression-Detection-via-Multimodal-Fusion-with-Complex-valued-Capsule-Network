...
if __name__ == '__main__':
    ...
    config_exp = {"batch_size":32,
                  ...
                  "epochs":250}
    x1_train, x2_train, y_train, x1_test, x2_test, y_test = train_test_split_done()
    model = LMF(configs_audio,configs_text,configs_fusion).to(config_exp["device"])
    ...
    for ep in range(1, config_exp['epochs']):
        ...
        if best_f1_score>0.9:
            torch.save(model,os.path.dirname(os.path.dirname(os.getcwd()))+"/models/LMF_models/fuse-"+str(np.round(best_f1_score,2)))