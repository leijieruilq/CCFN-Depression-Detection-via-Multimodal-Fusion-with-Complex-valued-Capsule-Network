...
from Crossclr_fusion import Crossclr
...
def train(epoch,model,config,xtrain1,xtrain2,ytrain,criterion,device):
    ...
    for i in tqdm(range(0, xtrain1.shape[0],config['batch_size'])):
        ...
        # 将模型的参数梯度设置为0
        optimizer.zero_grad()
        output,x_audio,x_text = model(x1,x2)
        pred = output.data.max(1, keepdim=True)[1]
        #print(pred.shape, y.shape)
        correct += pred.eq(y.data.view_as(pred)).cuda().sum()
        loss1,loss2 = criterion(output,y.long(),x_audio,x_text)
        # 后向传播调整参数
...
if __name__ == '__main__':
    ...
    config_exp = {"batch_size":24,
                  ...
                  "device":"cuda:4",
                  'learning_rate':1e-4,
                  'use_entropy':False, #使用自定义损失函数(为False)
                  "patience":200, #早停轮数
                  "epochs":250}
    ...
    """
    train_idx = np.load(os.path.dirname(os.path.dirname(os.getcwd()))+"/fold_3_train.npy")
    test_idx = np.load(os.path.dirname(os.path.dirname(os.getcwd()))+"/fold_3_test.npy")
    model = Crossclr(configs_audio,configs_text,configs_fusion).to(config_exp["device"])
    optimizer = optim.Adam(model.parameters(), lr=config_exp["learning_rate"])
    criterion = CrossCLR_onlyIntraModality()
    loss_text=[]
    acc=[]
    pre=[]
    recalls=[]
    ...
    for ep in range(1, config_exp['epochs']):
        ...
        if best_f1_score>0.9:
            torch.save(model,os.path.dirname(os.path.dirname(os.getcwd()))+"/models_3/CrossCLR_models/fuse-"+str(np.round(best_f1_score,3))+"-"+str(3))