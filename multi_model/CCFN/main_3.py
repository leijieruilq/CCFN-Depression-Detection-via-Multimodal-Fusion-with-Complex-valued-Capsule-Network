...
from CCFN_fusion import CCFN
from complex_caspnet import CapsuleLoss
import numpy as np
...
def train(epoch,model,config,xtrain1,xtrain2,ytrain,criterion,device):
    ...
    for i in tqdm(range(0, xtrain1.shape[0],config['batch_size'])):
        ...
        # 将模型的参数梯度设置为0
        optimizer.zero_grad()
        output,res,x_fusion = model(x1,x2)
        pred = output.data.max(1, keepdim=True)[1]
        #print(pred.shape, y.shape)
        correct += pred.eq(y.data.view_as(pred)).cuda().sum()
        loss1,loss2 = criterion(x_fusion,y.reshape(-1,1).long(),output,res,device)
        # 后向传播调整参数
...
if __name__ == '__main__':
    ...
    model = CCFN(configs_audio,configs_text,configs_fusion).to(config_exp["device"])
    ...
    for ep in range(1, config_exp['epochs']):
        ...
        if best_f1_score>0.9:
            torch.save(model,os.path.dirname(os.path.dirname(os.getcwd()))+"/models_3/CCFN_models/fuse-"+str(np.round(best_f1_score,2))+"-"+str(3))
