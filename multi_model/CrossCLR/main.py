...
if __name__ == '__main__':
    ...
    criterion = CrossCLR_onlyIntraModality(device=config_exp["device"])
    loss_text=[]
    acc=[]
    pre=[]
    recalls=[]
    ...
    for ep in range(1, config_exp['epochs']):
        ...
        if best_f1_score>0.9:
            torch.save(model,os.path.dirname(os.path.dirname(os.getcwd()))+"/models/CrossCLR_models/fuse-"+str(np.round(best_f1_score,2)))
